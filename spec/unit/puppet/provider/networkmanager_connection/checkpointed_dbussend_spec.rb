...
describe Puppet::Type.type(:networkmanager_connection).provider(:checkpointed_dbussend) do
  ...
  context 'with example configuration' do
    ...
    it 'uses nmcli to activate the connection, with a checkpoint taken and then removed after success' do
      ...
      allow(provider).to receive(:dbus_call).with(
        :Get,
        'string:org.freedesktop.NetworkManager.Checkpoint',
        'string:Created',
        dbus_object: '/org/freedesktop/NetworkManager/Checkpoint/1',
        method_service: 'org.freedesktop.DBus.Properties',
      )
    ...
    it 'uses nmcli to activate the connection, with a checkpoint taken and then rolled back on failure' do
      ...
      allow(provider).to receive(:dbus_call).with(
        ...
        'string:org.freedesktop.NetworkManager.Checkpoint',
        'string:Created',
        dbus_object: '/org/freedesktop/NetworkManager/Checkpoint/2',
        method_service: 'org.freedesktop.DBus.Properties',
      )
      service_mock = instance_double('Puppet::HTTP::Service::Puppetserver')
